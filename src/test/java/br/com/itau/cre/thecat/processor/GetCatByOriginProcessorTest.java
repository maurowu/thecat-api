...
public class GetCatByOriginProcessorTest {
	...
	@InjectMocks
	private GetCatByOriginProcessor getCatByOriginProcessor = new GetCatByOriginProcessor();
	...
	public void processTest() throws Exception {
		...
		Mockito.when(catsBreedsRepository.findByOrigin(Mockito.any())).thenReturn(opt);
		getCatByOriginProcessor.process(exchange);
	...
	public void processExceptionTest() throws Exception {
		...
		catsBreeds.add(CatsBreeds.builder().catId("10").build());
		List<CatsBreeds> opt = catsBreeds;
		BDDMockito.given(catsBreedsRepository.findByOrigin(Mockito.any())).willAnswer(invocation -> {
